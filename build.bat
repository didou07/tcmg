...
if not exist "%SRCDIR%\tcmg.c" (
    ...
    echo  Run this script from the repository root where src\ lives.
    echo.
    pause & exit /b 1
)

:: -- Read version from src\tcmg-globals.h ---------------------------------
set TCMG_VERSION=unknown
for /f "tokens=3 delims= " %%V in ('findstr /c:"#define TCMG_VERSION" "%SRCDIR%\tcmg-globals.h" 2^>nul') do (
    set TCMG_VERSION=%%~V
)
...
if !errorlevel! neq 0 (
    ...
    echo.
    echo  Install MinGW-w64 and add gcc.exe to PATH:
...
if "%DEBUG%"=="1" (
    set OPT_FLAGS=-Og -g
    set LD_EXTRA=
    echo  Mode: DEBUG (no optimisation, debug symbols included)
) else (
...
for %%F in (%SRCS%) do (
    ...
    if !errorlevel! neq 0 (
        echo.
        echo  ERROR: Failed to compile %SRCDIR%\%%F
        pause & exit /b 1
    )
    set OBJS=!OBJS! "!OBJ!"
)
