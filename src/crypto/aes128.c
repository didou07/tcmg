#define MODULE_LOG_PREFIX "aes128"
#include "crypto.h"
#include "aes128.h"

static const uint8_t sbox[256] = {

};

static const uint8_t rsbox[256] = {

};

static uint8_t gmul(uint8_t a, uint8_t b)
{
    uint8_t p = 0;
    while (b) {
        if (b & 1) p ^= a;
        a = (uint8_t)((a << 1) ^ ((a & 0x80) ? 0x1B : 0));
        b >>= 1;
    }
    return p;
}

static void key_expand(const uint8_t key[16], uint8_t rk[176])
{
    static const uint8_t rcon[10] = {1,2,4,8,16,32,64,128,27,54};
    int bytes = 16, rcon_idx = 0;
    memcpy(rk, key, 16);
    while (bytes < 176) {
        uint8_t t[4];
        memcpy(t, rk + bytes - 4, 4);
        if ((bytes % 16) == 0) {
            uint8_t x = t[0];
            t[0] = sbox[t[1]];
            t[1] = sbox[t[2]];
            t[2] = sbox[t[3]];
            t[3] = sbox[x];
            t[0] ^= rcon[rcon_idx++];
        }
        for (int i = 0; i < 4; i++) {
            rk[bytes] = (uint8_t)(rk[bytes - 16] ^ t[i]);
            bytes++;
        }
    }
}

static void add_round_key(uint8_t s[16], const uint8_t *rk)
{
    for (int i = 0; i < 16; i++) s[i] ^= rk[i];
}

static void sub_bytes(uint8_t s[16])
{
    for (int i = 0; i < 16; i++) s[i] = sbox[s[i]];
}

static void inv_sub_bytes(uint8_t s[16])
{
    for (int i = 0; i < 16; i++) s[i] = rsbox[s[i]];
}

static void shift_rows(uint8_t s[16])
{
    uint8_t t[16];
    memcpy(t, s, 16);
    s[0]=t[0];  s[4]=t[4];  s[8]=t[8];   s[12]=t[12];
    s[1]=t[5];  s[5]=t[9];  s[9]=t[13];  s[13]=t[1];
    s[2]=t[10]; s[6]=t[14]; s[10]=t[2];  s[14]=t[6];
    s[3]=t[15]; s[7]=t[3];  s[11]=t[7];  s[15]=t[11];
}

static void inv_shift_rows(uint8_t s[16])
{
    uint8_t t[16];
    memcpy(t, s, 16);
    s[0]=t[0];  s[4]=t[4];  s[8]=t[8];   s[12]=t[12];
    s[1]=t[13]; s[5]=t[1];  s[9]=t[5];   s[13]=t[9];
    s[2]=t[10]; s[6]=t[14]; s[10]=t[2];  s[14]=t[6];
    s[3]=t[7];  s[7]=t[11]; s[11]=t[15]; s[15]=t[3];
}

static void mix_columns(uint8_t s[16])
{
    for (int c = 0; c < 4; c++) {
        uint8_t *p = s + c * 4;
        uint8_t a=p[0], b=p[1], c2=p[2], d=p[3];
        p[0]=(uint8_t)(gmul(a,2)^gmul(b,3)^c2^d);
        p[1]=(uint8_t)(a^gmul(b,2)^gmul(c2,3)^d);
        p[2]=(uint8_t)(a^b^gmul(c2,2)^gmul(d,3));
        p[3]=(uint8_t)(gmul(a,3)^b^c2^gmul(d,2));
    }
}

static void inv_mix_columns(uint8_t s[16])
{
    for (int c = 0; c < 4; c++) {
        uint8_t *p = s + c * 4;
        uint8_t a=p[0], b=p[1], c2=p[2], d=p[3];
        p[0]=(uint8_t)(gmul(a,14)^gmul(b,11)^gmul(c2,13)^gmul(d,9));
        p[1]=(uint8_t)(gmul(a,9)^gmul(b,14)^gmul(c2,11)^gmul(d,13));
        p[2]=(uint8_t)(gmul(a,13)^gmul(b,9)^gmul(c2,14)^gmul(d,11));
        p[3]=(uint8_t)(gmul(a,11)^gmul(b,13)^gmul(c2,9)^gmul(d,14));
    }
}

static void encrypt_block(uint8_t s[16], const uint8_t rk[176])
{
    add_round_key(s, rk);
    for (int r = 1; r < 10; r++) {
        sub_bytes(s); shift_rows(s); mix_columns(s); add_round_key(s, rk + r * 16);
    }
    sub_bytes(s); shift_rows(s); add_round_key(s, rk + 160);
}

static void decrypt_block(uint8_t s[16], const uint8_t rk[176])
{
    add_round_key(s, rk + 160);
    for (int r = 9; r > 0; r--) {
        inv_shift_rows(s); inv_sub_bytes(s); add_round_key(s, rk + r * 16); inv_mix_columns(s);
    }
    inv_shift_rows(s); inv_sub_bytes(s); add_round_key(s, rk);
}

void aes128_ecb_encrypt(const uint8_t key[16], uint8_t *data, size_t len)
{
    if (!key || !data || (len % 16) != 0) return;
    uint8_t rk[176];
    key_expand(key, rk);
    for (size_t i = 0; i < len; i += 16) encrypt_block(data + i, rk);
    secure_zero(rk, sizeof(rk));
}

void aes128_ecb_decrypt(const uint8_t key[16], uint8_t *data, size_t len)
{
    if (!key || !data || (len % 16) != 0) return;
    uint8_t rk[176];
    key_expand(key, rk);
    for (size_t i = 0; i < len; i += 16) decrypt_block(data + i, rk);
    secure_zero(rk, sizeof(rk));
}
