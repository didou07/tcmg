...
/* Platform detection — define TCMG_OS_WINDOWS before including this header
 * to force the Windows path (e.g. when cross-compiling).  Otherwise it is
 * auto-detected here once and used consistently throughout the codebase.  */
...
/* ── Safe string helpers ────────────────────────────────────────────────────
 * tcmg_strlcpy / tcmg_strlcat: always NUL-terminate, return bytes written.
 * Use these instead of bare strncpy (which may leave no NUL) throughout.   */
