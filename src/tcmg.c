                          ...
                          uint16_t sid, uint16_t mid, uint32_t pid)
{
	...
	{ size_t hlen = strlen(hash);
	  for (i = 0; i < 14; i++)
	      cl->session_key[i] ^= (uint8_t)hash[i % hlen]; }
                        ...
                        uint16_t caid_hdr)
{
	...
	if (ch)
		tcmg_strlcpy(cl->last_channel, ch, sizeof(cl->last_channel));
	else
		cl->last_channel[0] = '\0';
...
int main(int argc, char *argv[])
{
	...
	while (g_running)
	{
		...
		/* Atomically check-and-increment: reject if already at limit */
		...
		if (!args)
		{
			__sync_fetch_and_sub(&g_active_conns, 1);
			close(cfd);
			tcmg_log("out of memory -- rejected connection");
			continue;
		}
		args->fd = cfd;
	...
	for (int _w = 0; _w < 50 && g_active_conns > 0; _w++)
	{
		...
		/* sleep() is defined in globals as Sleep(1000) on Windows,
		 * nanosleep(1s) path not needed -- 1-second granularity is fine. */
		sleep(1);
	}
	if (g_active_conns > 0)
