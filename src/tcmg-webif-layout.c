                ...
                const char *title, const char *active)
{
	...
	pos = buf_printf(buf,bsz,pos,
		...
		"});"
		/* Unified poll - runs on every page, drives both topbar and status widgets */
		"var _pm=(function(){"
		"  var v=parseInt(sessionStorage.tcmg_poll)||%d;"
		"  var el=document.getElementById('ps_');if(el)el.value=v;"
		"  return v*1000;"
		"})();"
		...
		"function _fmt(n){return n>=1e6?(n/1e6).toFixed(1)+'M':n>=1e3?(n/1e3).toFixed(1)+'K':String(n);}"
		"function _anim(id,v){"
		"  var e=document.getElementById(id);if(!e)return;"
		"  if(e.textContent===String(v))return;"
		"  e.textContent=v;e.classList.remove('cnt-up');void e.offsetWidth;e.classList.add('cnt-up');"
		"}"
		...
		"  _ut=d.uptime_s|0;"
		"  if(!_ut_tmr){_ut_tmr=setInterval(function(){"
		"    _ut++;var e=document.getElementById('p_up');if(e)e.textContent=_fmt_up(_ut);"
		"  },1000);}"
		"  var eu=document.getElementById('p_up');if(eu)eu.textContent=_fmt_up(_ut);"
		...
		"  var tb=document.getElementById('p_clients');if(!tb)return;"
		"  if(!d.clients||!d.clients.length){"
		"    tb.innerHTML='<tr class=\"erow\"><td colspan=\"8\">No active connections</td></tr>';return;"
		"  }"
		"  var ids={};"
		"  d.clients.forEach(function(cl){ids[cl.thread_id]=1;});"
		/* Remove rows for gone clients */
		"  Array.from(tb.querySelectorAll('tr[id^=\"row_\"]')).forEach(function(r){"
		"    var tid=r.id.slice(4);"
		"    if(!ids[tid]&&r.style.opacity!=='.4'){r.style.opacity='.4';"
		...
		"    tr.className='nw';tr.id='row_'+cl.thread_id;"
		"    tr.innerHTML="
		...
		"  if(r){r.style.opacity='.4';setTimeout(function(){if(r.parentNode)r.parentNode.removeChild(r);},800);}"
		"}"
		"function _poll(){"
		"  if(_busy)return;"
		"  _busy=true;"
