...
/* ────────────────────────────────────────────────────────────────────────────
 * tcmg-webif-common.c  —  HTTP primitives shared by all webif translation units
 *
 * Sections:
 *   1. Session management
 *   2. String helpers  (html_escape, json_escape, url_decode)
 *   3. HTTP request parsing
 *   4. Dynamic response buffer (buf_printf)
 *   5. URL / form helpers
 *   6. HTTP response helpers
 *   7. Base64 + auth
 *   8. Aggregated server stats
 * ────────────────────────────────────────────────────────────────────────────*/
...
/*
 * html_escape — convert <, >, & to HTML entities in-place safe copy.
 * Returns number of bytes written (excl. NUL).  dstsz must include space
...
/*
 * html_escape_alloc — heap-allocate an HTML-escaped copy of src.
 * Caller must free().  Returns NULL on OOM.
 * *truncated set to 1 if src was longer than maxbytes.
 */
...
/*
 * file_read_escaped — read a file into a heap-allocated HTML-escaped string.
 * Caller must free().  Returns empty-string alloc (not NULL) if file missing.
 * *truncated set if file exceeds maxbytes.
 */
...
/*
 * json_escape — copy src into dst with JSON string escaping (no surrounding
 * quotes).  Returns bytes written.
 */
...
/*
 * url_decode — decode %XX and + in-place (single canonical implementation).
 */
