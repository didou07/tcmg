...
#include "webif_assets.h"   /* TCMG_CSS, TCMG_JS — run: make assets */

/* Icon macros (C string literals — safe to use anywhere) */
#define ICO_LOGO \
...
{
    /* Map restart/shutdown sub-pages to "power" for nav highlight */
    ...
    /* Nav links — defined as a plain array of structs */
    typedef struct { int sep; const char *id, *href, *icon, *label; } t_nav;
    /* SVG strings that can't be #defines inside struct initializers */
    ...
    /* TCMG_JS contains %d and %% format specifiers — pass as fmt string */
    pos = buf_printf(buf, bsz, pos,
