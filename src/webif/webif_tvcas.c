...
/* Embedded HTML/TCMG_CSS/JS string literals may exceed ISO C99's minimum 4095-char
 * limit, which is intentional for a web interface. Suppress the pedantic warning
...
void send_page_tvcas(int fd)
{
	...
	/* Page-level extra TCMG_CSS that extends (not replaces) the global theme */
	pos = buf_printf(&buf, &bsz, pos,
		"<style>"
		/* Tab strip */
		".tv-tabs{display:flex;gap:2px;margin-bottom:20px;"
		"background:var(--s2);border:1px solid var(--bd);"
		"border-radius:var(--rsm);padding:4px}"
		".tv-tab{flex:1;padding:8px 16px;cursor:pointer;font-size:12.5px;"
		"font-weight:600;font-family:var(--sans);letter-spacing:.03em;"
		"color:var(--t1);border-radius:var(--rxs);border:1px solid transparent;"
		"background:none;transition:all .18s}"
		".tv-tab.active{background:var(--s4);color:var(--p);border-color:var(--pg)}"
		".tv-tab:hover:not(.active){color:var(--t0);background:var(--s3)}"
		".tv-panel{display:none}.tv-panel.active{display:block}"

		/* Input cards */
		".tv-card{background:var(--s1);border:1px solid var(--bd);"
		"border-radius:var(--r);padding:18px 20px;margin-bottom:14px;"
		"animation:fu .3s var(--ease) both}"

		/* Labels */
		".tv-lbl{font-size:10px;font-weight:700;text-transform:uppercase;"
		"letter-spacing:.10em;color:var(--t1);margin-bottom:5px;margin-top:12px}"
		".tv-lbl:first-child{margin-top:0}"

		/* Inputs */
		".tv-inp{width:100%%;background:var(--s2);border:1px solid var(--bd2);"
		"border-radius:var(--rsm);color:var(--t0);font-family:var(--mono);"
		"font-size:12.5px;padding:8px 12px;outline:none;"
		"transition:border-color .18s;box-sizing:border-box}"
		".tv-inp:focus{border-color:var(--pg);box-shadow:0 0 0 3px var(--ps)}"
		...
		".tv-res-empty{padding:14px 16px;font-size:12px;color:var(--t2);"
		"font-family:var(--mono)}"

		/* Result table */
		".tv-tbl{width:100%%;border-collapse:collapse}"
		".tv-tbl tr{border-bottom:1px solid var(--bd)}"
		".tv-tbl tr:last-child{border-bottom:none}"
		".tv-tbl td{padding:9px 14px;font-family:var(--mono);font-size:12.5px;"
		"vertical-align:middle}"
		".tv-tbl td.tk{color:var(--t2);font-size:10px;font-weight:700;"
		"text-transform:uppercase;letter-spacing:.10em;white-space:nowrap;"
		"width:110px;border-right:1px solid var(--bd);background:var(--s1)}"
		".tv-tbl td.tv{color:var(--t0);padding-left:16px;word-break:break-all}"

		/* Split layout (two columns) */
		".tv-split{display:flex;gap:0;border-bottom:1px solid var(--bd)}"
		".tv-split-box{flex:1;border-right:1px solid var(--bd)}"
		".tv-split-box:last-child{border-right:none}"
		".tv-split-hdr{font-size:9px;font-weight:700;letter-spacing:.14em;"
		"text-transform:uppercase;color:var(--p);background:var(--s1);"
		"padding:6px 14px;border-bottom:1px solid var(--bd)}"

		/* Radio group */
		".tv-rgrp{display:flex;align-items:center;gap:14px;margin-bottom:12px;"
		"flex-wrap:wrap}"
		".tv-rgrp label{display:flex;align-items:center;gap:6px;"
		"cursor:pointer;font-size:13px;font-weight:500;color:var(--t0)}"
		".tv-rgrp span{font-size:10px;font-weight:700;letter-spacing:.10em;"
		...
		".tv-er{color:var(--re)}"
		".tv-hi{color:var(--cy)}"
		".tv-cw{font-family:var(--mono);font-size:13px;font-weight:700;"
		"color:var(--cy);letter-spacing:.4px;word-break:break-all}"
		".tv-ts{color:var(--t0)}"
		".tv-dim{color:var(--t2)}"
		"</style>");
	...
	pos = buf_printf(&buf, &bsz, pos,
		"<div class='tv-tabs'>"
		"<button class='tv-tab active' onclick='tvTab(0)'>"
		"<svg width='13' height='13' viewBox='0 0 24 24' fill='none' "
		"stroke='currentColor' stroke-width='2' style='vertical-align:-2px;margin-right:5px'>"
		"<rect x='3' y='11' width='18' height='11' rx='2'/>"
		"<path d='M7 11V7a5 5 0 0 1 10 0v4'/>"
		"</svg>ECM Decryptor</button>"
		"<button class='tv-tab' onclick='tvTab(1)'>"
		"<svg width='13' height='13' viewBox='0 0 24 24' fill='none' "
		"stroke='currentColor' stroke-width='2' style='vertical-align:-2px;margin-right:5px'>"
	...
	pos = buf_printf(&buf, &bsz, pos,
		...
		"</div>"
		"<div class='tv-lbl'>ECM — 110 hex chars (header 80 or 81 selects key half)</div>"
		...
		"</div>"
		"<button class='tv-btn' onclick='tvDec()'>"
		"<svg width='14' height='14' viewBox='0 0 24 24' fill='none' stroke='currentColor' stroke-width='2'>"
		"<rect x='3' y='11' width='18' height='11' rx='2'/><path d='M7 11V7a5 5 0 0 1 10 0v4'/>"
		"</svg>Decrypt ECM</button>"
		"</div>"
		"<div class='tv-card'>"
		"<div class='tv-lbl' style='margin-top:0'>Result</div>"
		"<div id='ecm_res' class='tv-res'>"
		"<div class='tv-res-empty tv-dim'>— run decryption to see result —</div>"
		"</div>"
		"</div>"
		"</div>");
	...
	pos = buf_printf(&buf, &bsz, pos,
		...
		"<div class='tv-rgrp'>"
		"<span>Direction</span>"
		"<label><input type='radio' name='cv_d' value='3to4' onchange='tvDC()' checked>3 &rarr; 4</label>"
		"<label><input type='radio' name='cv_d' value='4to3' onchange='tvDC()'>4 &rarr; 3</label>"
		"</div>"
		"<div id='cv_il' class='tv-lbl'>TVCAS3 Key — 32 or 64 hex chars</div>"
		"<input id='cv_in' class='tv-inp' type='text' placeholder='Key hex...'>"
		"<button class='tv-btn' onclick='tvConv()'>"
		"<svg width='14' height='14' viewBox='0 0 24 24' fill='none' stroke='currentColor' stroke-width='2'>"
		...
		"</div>"
		"<div class='tv-card'>"
		"<div id='cv_ol' class='tv-lbl' style='margin-top:0'>TVCAS4 Key</div>"
		"<div id='cv_res' class='tv-res'>"
		"<div class='tv-res-empty tv-dim'>— run conversion to see result —</div>"
		"</div>"
		"</div>"
		"</div>");
	...
	pos = buf_printf(&buf, &bsz, pos,
		...
		"for(let i=0;i<tabs.length;i++)if(tabs[i].classList.contains('active')){sessionStorage.setItem('tv_tab',i);break;}}\n"
		...
		"const ok=r.csOk?'<span class=tv-ok>\u2713 Valid</span>':'<span class=tv-er>\u2717 Mismatch</span>';"
		...
		"+row('CW','<span class=tv-cw>'+r.cw+'</span>')"
		"+'</table></div>'"
		"+'</div>'"
		");"
		...
		"return sr('cv_res','<div class=tv-res-empty><span class=tv-er>Invalid length (32 or 64 hex chars)</span></div>');"
		"const d=document.querySelector('input[name=cv_d]:checked').value;"
		"const lbl=d==='3to4'?'TVCAS4 Key':'TVCAS3 Key';"
		"sr('cv_res',"
		"'<div class=tv-split-hdr>'+lbl+'</div>'"
		"+'<table class=tv-tbl>'"
		"+row('Key','<span class=tv-cw>'+convKey(k,d==='3to4')+'</span>')"
		"+'</table>'"
		");"
