                             ...
                             uint8_t *cw, const S_ACCOUNT *acc)
{
	...
	if (ecm[4] < 2) return EMU_NOT_SUPPORTED;
	uint8_t slen = ecm[4] - 2;
	uint8_t nano = ecm[5];

	/* We only handle nano=0x64 with exactly 48 bytes of payload    */
	if (slen != 48 || nano != 0x64) return EMU_NOT_SUPPORTED;

	/* Verify the full packet is long enough: header(7) + slen      */
	if (len < 7 + (int32_t)slen) return EMU_NOT_SUPPORTED;
