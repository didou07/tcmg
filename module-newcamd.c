...
static volatile int32_t  s_ncd_running = 0;
...
static void cw_cache_store(const uint8_t *ecm_md5, const uint8_t *cw)
{
	...
	e->valid = 1;
	pthread_mutex_unlock(&g_cw_cache_mtx[shard]);
	tcmg_log_dbg(D_NEWCAMD, "cw cache stored slot=%u shard=%u", idx, shard);
}
                              ...
                              uint16_t sid, uint16_t mid, uint32_t pid)
{
	...
	{
		ncd_nak(cl, sid, mid, pid);
		tcmg_log("%s LOGIN failed: malformed user field (ulen=%zu umax=%zu)",
		         ip, ulen, umax);
		return false;
	}
	hash = user + ulen + 1;
                             ...
                             uint16_t caid_hdr)
{
	...
	log_cw_result(ecm_caid, sid, dlen, cw, res == EMU_OK, (int32_t)ms, cl->user);
